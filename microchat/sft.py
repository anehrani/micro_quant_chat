...
def sft_btc(
    *,
    ...
    """Stage 2: supervised fine-tuning (teacher forcing) on BTC tokens only."""
    ...
    for e in range(epochs):
        ...
        if va < best:
            ...
            print(f"  saved best sft checkpoint (val={best:.4f})")

    total = time.time() - start
    print(f"SFT complete in {total:.2f}s | best val {best:.4f}")


def main() -> None:
    import argparse

    p = argparse.ArgumentParser(description="Stage 2: supervised fine-tuning on BTC")
